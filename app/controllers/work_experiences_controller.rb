  ...
  def update
    respond_to do |format|
      if @work_experience.update(work_experience_params)
        format.turbo_stream { render turbo_stream: turbo_stream.replace("work_experience_item#{@work_experience.id}", partial: 'work_experiences/work_experience', locals: { work_experience: @work_experience }) }
      else
        format.turbo_stream { render turbo_stream: turbo_stream.replace('remote_modal', partial: 'shared/turbo_modal', locals: { form_partial: 'work_experiences/form', modal_title: 'Edit new work experience'})}
      end
    end
  end
